...
function fetchData() {
	...
	FetchPending = $.ajax({ url: 'data/aircraft.json',
		...
		refreshSelected();
                ...
                if (LastReceiverTimestamp === now) {
                        ...
                        if (StaleReceiverCount > 5) {
                                $("#update_error_detail").text("The data from Newton hasn't been updated in a while. Maybe Newton is no longer running?");
        ...
        FetchPending.fail(function(jqxhr, status, error) {
                $("#update_error_detail").text("AJAX call failed (" + status + (error ? (": " + error) : "") + "). Maybe Newton is no longer running?");
                $("#update_error").css('display','block');
        });
}
...
function refreshSelected() {
	...
	if (typeof SelectedPlane !== 'undefined' && SelectedPlane != "ICAO" && SelectedPlane != null) {
        ...
        if (!selected) {
                ...
                $('#newton_infoblock').css('display','block');
                $('#newton_version').text(Dump1090Version);
                $('#newton_total_ac').text(TrackedAircraft);
                $('#newton_total_ac_positions').text(TrackedAircraftPositions);
                $('#newton_total_history').text(TrackedHistorySize);

                if (MessageRate !== null) {
                        $('#newton_message_rate').text(MessageRate.toFixed(1));
                } else {
                        $('#newton_message_rate').text("n/a");
                }

                return;
        }
        
        $('#newton_infoblock').css('display','none');
