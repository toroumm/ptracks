...
import ptracks.view.super.dlg_config as wmain
...
class CViewSuper(view.CViewManager):
    """
    the interface to configuration. Handles all interaction with user
    """
    # ---------------------------------------------------------------------------------------------
    def __init__(self, f_control, f_model):
        ...
        # init super class
        super(CViewSuper, self).__init__(f_control)
    ...
    def run(self):
        ...
        # cria a visualização
        l_wmain = wmain.CDlgConfig(self.control)
        assert l_wmain

        # exibe o supervisor de simulação
        l_wmain.show()
